...
export const BlogInfo = () => {
  ...
  const getAllBlogs = async () => {
    setloading(true);
    try {
      const productTemp = await getDoc(doc(firestore, "blogPost", params.id))
  ...
  const addComment = async () => {
    const commentRef = collection(firestore, "blogPost/" + `${params.id}/` + "comment")
    try {
      await addDoc(
        commentRef, {
        fullName,
  ...
  const getcomment = async () => {
    try {
      const q = query(
        collection(firestore, "blogPost/" + `${params.id}/` + "comment/"),
        orderBy('time')
      );
