...
export default function SearchDialog() {
    ...
    const { mode, searchkey, setSearchkey, getAllBlogs } = context;
            ...
            <Dialog className=" relative right-[1em] w-[25em]  md:right-0 md:w-0 lg:right-0 lg:w-0" open={open} handler={handleOpen} style={{ background: mode === 'light' ? '#2f3542' : '#2f3542', color: mode === 'dark' ? 'white' : 'black' }}>
                ...
                <DialogBody >
                    ...
                    <div className="flex justify-center flex-wrap  sm:mx-auto sm:mb-2 -mx-2  mt-4 mb-2 ">
                        {
                            getAllBlogs?.filter((obj) => obj.blogs.title.toLowerCase().includes(searchkey)).map((item, index) => {
                                console.log(item);
                                const { thumbnail, date, id } = item;
                                return (
                                    <div key={index} className="p-2 sm:w-1/4 w-full " >
                                        <div onClick={() => naviagte(`/bloginfo/${id}`)} className=" container cursor-pointer mx-auto px-4 bg-gray-200 p-2 rounded-lg ">
                                            {/* Blog Thumbnail  */}
                    ...
                    <div className=" text-center">
                        <h1 className=" text-gray-600">Powered By  Muhammad Umar</h1>
                    </div>
                </DialogBody>
            </Dialog>
        </Fragment>
